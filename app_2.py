...
with open("data_2.json", "r", encoding="utf-8") as f:
    ...
    documents = act_data["act"]["sections"]
...
# สร้าง Embedding และ FAISS Index โดยใช้เฉพาะข้อความ (text) และเลขมาตรา (section_number)
contents = [doc["text"] for doc in documents]
articles = [doc["section_number"] for doc in documents]  # ดึงข้อมูลมาตรา
...
def query():
    try:
        ...
        # Generate คำตอบ
        outputs = model.generate(inputs["input_ids"], max_length=1800, temperature=0.5, top_p=0.9)
